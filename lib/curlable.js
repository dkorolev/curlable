...
class Curlable {
  constructor(cmdline, options) {
    ...
    this.logger('Spawned `/bin/bash` with `' + cmdline + '`, PID ' + this.spawnedProcess.pid + '\n');
    ...
    this.spawnedProcess.on('close', (code) => {
      ...
      this.logger('Spawned process exited with code ' + code + '.\n');
    ...
    readStreamByLines(
      ...
      () => {
        this.logger('Spawned process output stream has ended.\n');
